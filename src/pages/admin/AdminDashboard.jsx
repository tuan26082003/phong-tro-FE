...
export default function AdminDashboard() {
  const [stats, setStats] = useState({
    ...
    totalTenants: 0,
    totalRevenue: 0,
    totalRenters: 0, // Người đăng ký
    totalOwners: 0,  // Chủ trọ
  });

  const [selectedMonth, setSelectedMonth] = useState(dayjs().format("YYYY-MM"));
  const [selectedRoom, setSelectedRoom] = useState(null);
  const [selectedMonths, setSelectedMonths] = useState(6); // 3, 6, 9, 12
  const [viewMode, setViewMode] = useState("room"); // "room" or "owner"
  ...
  useEffect(() => {
    if (selectedMonth && rooms.length > 0 && viewMode === "room") {
      loadRoomRevenue();
    }
  }, [selectedMonth, selectedRoom, rooms, viewMode]);

  useEffect(() => {
    if (selectedMonth && owners.length > 0 && viewMode === "owner") {
      loadOwnerRevenue();
    }
  }, [selectedMonth, selectedOwner, owners, viewMode]);
  ...
  const loadStats = async () => {
    try {
      ...
      // Lấy số người đăng ký (RENTER)
      let totalRenters = 0;
      try {
        const rentersRes = await axiosClient.get("/api/user/role/RENTER", {
          params: { page: 0, size: 999 },
        });
        const renterData = rentersRes.data.data || rentersRes.data.content || [];
        totalRenters = Array.isArray(renterData) ? renterData.length : rentersRes.data.totalElements || 0;
      } catch (err) {
        console.error("Load renters error:", err);
      }

      // Lấy số chủ trọ (OWNER)
      let totalOwners = 0;
      try {
        const ownersRes = await axiosClient.get("/api/user/role/OWNER", {
          params: { page: 0, size: 999 },
        });
        const ownerData = ownersRes.data.data || ownersRes.data.content || [];
        totalOwners = Array.isArray(ownerData) ? ownerData.length : ownersRes.data.totalElements || 0;
      } catch (err) {
  ...
  const loadOwners = async () => {
    try {
      ...
      const ownerList = res.data.data || res.data.content || [];
  ...
  const loadOwnerRevenue = async () => {
    try {
      // Lấy doanh thu của tất cả owners trong tháng đã chọn
      const revenuePromises = owners.map(async (owner) => {
        try {
          ...
          const revenue = res.data.data || res.data || 0;
          return {
            ownerName: owner.fullName || owner.username || `Owner #${owner.id}`,
            ownerId: owner.id,
            revenue: Number(revenue) || 0,
          };
        } catch (err) {
          console.error(`Load revenue for owner ${owner.id} error:`, err);
          return {
            ownerName: owner.fullName || owner.username || `Owner #${owner.id}`,
            ownerId: owner.id,
            revenue: 0,
      ...
      let results = await Promise.all(revenuePromises);

      // Lọc theo owner đã chọn nếu có
      if (selectedOwner) {
        results = results.filter((r) => r.ownerId === selectedOwner);
      }

      setOwnerRevenue(results);
    } catch (err) {
      console.error("Load owner revenue error:", err);
      setOwnerRevenue([]);
    }
  };
...
const pieConfig = {
  ...
  return (
    <div style={{ padding: "24px" }}>
      ...
      {/* DOANH THU CHI TIẾT THEO PHÒNG / CHỦ TRỌ */}
      <Row gutter={[16, 16]}>
        <Col span={24}>
          <Card
            ...
            extra={
              <div style={{ display: "flex", gap: 12 }}>
                <Select
                  ...
                  style={{ width: 150 }}
                >
                  <Select.Option value="room">Theo phòng</Select.Option>
                  <Select.Option value="owner">Theo chủ trọ</Select.Option>
                </Select>
                <DatePicker
                  picker="month"
                ...
                {viewMode === "room" ? (
                  ...
                  <Select
                    placeholder="Chọn chủ trọ (tất cả)"
                    allowClear
                    style={{ width: 200 }}
                    value={selectedOwner}
          ...
          >
            <Table
              columns={viewMode === "room" ? revenueColumns : ownerRevenueColumns}
              dataSource={viewMode === "room" ? roomRevenue : ownerRevenue}
              rowKey={(record) => record.roomId || record.ownerId}
