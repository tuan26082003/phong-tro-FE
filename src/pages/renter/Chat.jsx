...
export default function Chat() {
  ...
  useEffect(() => {
    if (currentUserId) {
      ...
      // Kiểm tra nếu có yêu cầu chat với user cụ thể
      const chatWithUserId = localStorage.getItem("chatWithUserId");
      if (chatWithUserId) {
        // Xóa khỏi localStorage để không tự động tạo lại
        localStorage.removeItem("chatWithUserId");
        
        // Tạo conversation với user đó
        createConversation(Number(chatWithUserId));
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentUserId]);
